...
@tf.custom_gradient
def correction_multiplication(correction_batch, loss_batch):
    result = correction_batch * loss_batch  

    def grad(upstream):
        dz_dx = 0
        dz_dy = correction_batch
...
class QMC_optimization_framework():
    ...
    def set_training_parameters(self, opt_mode, optimizer, batch_size, learning_rate,
                                stopping_criteria, max_number, correction_function=None):
        # opt_mode: 'GD' or 'HIG'
        ...
        if self.correction_function is not None: self.correction = True


    def computation(self,x_batch,y_batch):
        control_batch = self.model(y_batch)
        y_prediction_batch = self.solver(x_batch,control_batch)
        loss, loss_batch, ip_batch = self.loss_function(y_batch,y_prediction_batch)
        return loss, loss_batch, ip_batch, y_prediction_batch
    ...
    def compute_derivatives(self,x_batch,y_batch):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss, loss_batch, ip_batch, y_prediction_batch = self.computation(x_batch,y_batch)

            if self.correction == True:
                correction_batch = self.correction_function(ip_batch)
                scaled_loss_batch = correction_multiplication(correction_batch,loss_batch)
                scaled_loss = tf.reduce_sum(scaled_loss_batch)
            else:
                scaled_loss = loss

            loss_per_dp = scaled_loss / self.batch_size


        if self.opt_mode == 'GD':
            grad = tape.gradient(loss_per_dp, self.model.trainable_variables)
            return grad
        elif self.opt_mode == 'HIG':
    ...
    def process_derivatives(self,derivatives):
        if self.opt_mode == 'GD':
            ...
            flat_jacy_list = [tf.reshape(jac, (self.batch_size * self.y_dim * 2, -1)) for jac in jacy_batch]
